...
export async function POST(req: Request) {
  // From: apps/frontend/app/api/voice-advanced/route.ts
  // To:   apps/frontend/server/mcp/mcp-direct.ts
  const { invokeMcpToolDirect } = await import('../../../server/mcp/mcp-direct');
  try {
    const { tool, parameters } = await req.json();
    if (!tool) {
      return new Response(JSON.stringify({ success: false, error: 'tool is required' }), { status: 400 });
    }
